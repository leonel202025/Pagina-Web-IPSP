...
export function VerProfesores() {
  ...
  const [gradosMateriasSeleccionados, setGradosMateriasSeleccionados] = useState([]);
  ...
  const toggleGradoMateria = (id_grado, id_asignatura) => {
    ...
    if (exists) {
      setGradosMateriasSeleccionados(prev =>
        prev.filter(gm => gm.id_grado !== id_grado || gm.id_asignatura !== id_asignatura)
      );
    } else {
      setGradosMateriasSeleccionados(prev => [...prev, { id_grado, id_asignatura }]);
    }
  };
  ...
  const handleGuardarCambios = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/usuarios/profesores/${actualizado.id}`, {
        method: "PUT",
      ...
      if (res.ok) {
        // refrescar lista
        const resProfes = await fetch("http://localhost:5000/api/usuarios/profesores");
  ...
  const confirmarEliminarProfesor = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/usuarios/profesores/${profesorAEliminar.id}`, {
        method: "DELETE"
      });
      if (res.ok) {
        setProfesores(prev => prev.filter(p => p.id !== profesorAEliminar.id));
      ...
      <tbody>
        {profesores.map(profesor => (
          <tr key={profesor.id}>
            ...
            <td>
              {profesor.gradosMaterias?.map(gm => {
                const grado = grados.find(g => g.id === gm.id_grado)?.grado;
                const materia = asignaturas.find(a => a.id === gm.id_asignatura)?.asignatura;
        ...
        <div className="modal-profesor__container">
          ...
          >
            <div className="form-datos-personales">
              <label className="label__datos"><strong>Datos Personales</strong></label>
              <div className="row">
                <input
                  name="dni"
                  value={profesorAEditar.dni}
                  onChange={(e) =>
                    setProfesorAEditar({ ...profesorAEditar, dni: e.target.value })
                  }
                  placeholder="DNI"
                  required
                />
                <input
                  ...
                  onChange={(e) =>
                    setProfesorAEditar({ ...profesorAEditar, nombre: e.target.value })
                  }
                  placeholder="Nombre"
                  required
                />
              ...
              <div className="row">
                <input
                  ...
                  onChange={(e) =>
                    setProfesorAEditar({ ...profesorAEditar, email: e.target.value })
                  }
                  placeholder="Email"
                  required
                />
            ...
            <div className="form-asignaciones">
              <label className="label__datos"><strong>Asignaciones (Grado → Materia)</strong></label>
